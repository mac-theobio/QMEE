...
```{r parasiteSum.R}
(parasiteTable |> mutate(across(where(is.character), as.factor)) 
	|> summary()
)
```
...
* Keys link diferent tables
	* The gender, height, birthdate fields from the Individual table can be linked to the Event table when needed
...
## Long data is often tidier

* Use long structure to mark things that are comparable, or have the same units
* We often use ```pivot_longer``` to make things tidier

```{r}
longsim <- pivot_longer(sim, cols=!time, names_to="class", values_to="people")
print(longsim)
```

## Wide data is often more readable
* `spread` has been replaced by pivot_wider

```{r}
print(pivot_wider(longsim, names_from=class, values_from=people))
```

Also necessary for many analyses
...
```{r echo=FALSE}
religTab <- (
	read.table("../data/religion_basic.ccsv"
