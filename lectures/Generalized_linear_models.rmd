...
sscrape <- function(string="logistic+regression") {
    ...
    ## from https://stackoverflow.com/a/72254415/190277
...
logpois_pct <- round(sum(gscrape[c("logistic+regression", "Poisson+regression")])/sum(gscrape)*100)
```       
...
- most GLMs (`r logpois_pct`%) are logistic or Poisson

<!-- ## Most GLMs are logistic regression -->


```{r gscrapepix,message=FALSE,fig.width=6,fig.height=3, eval = FALSE}
...
> 	- with $\beta=\{1,1\}$, $T=15$:
> 	- counts $\sim \textrm{Poisson}(\lambda=\exp(1 + 1 \cdot (15-20)))$
> 	- = $\textrm{Poisson}(\lambda = \exp(-4))=\textrm{Poisson}(\lambda=`r round(exp(-4),3)`)$
...
>     - binary responses
> 	- families/models that already estimated dispersion (Gaussian, Gamma, neg binom, ...)
...
- effect on the prob scale **depends on baseline probability**
 ...
 - also see [UCLA FAQ on odds ratios](https://stats.idre.ucla.edu/other/mult-pkg/faq/general/faq-how-do-i-interpret-odds-ratios-in-logistic-regression/); @gelmanDataAnalysisUsing2006 (p. 82; [Google books link](https://books.google.ca/books?id=lV3DIdV0F9AC&printsec=frontcover&dq=gelman+hill+logistic+parameter&hl=en&sa=X&ved=0ahUKEwjt8Zq_1-bgAhUqzIMKHTl0D3EQ6AEIKjAA#v=snippet&q=divide%20by%204&f=false))
...
- don't use binomial/Poisson models with non-integer responses
- don't equate negative binomial with binomial rather than Poisson
- don't leave out `family=` in `glm()`; don't use `glm()` for linear models
- don't get confused about predictions on effect scale
- don't worry about overdispersion unnecessarily (e.g. for binary data)
- don't back-transform SEs rather than CIs
- don't ignore random effects (next topic -- mixed models)
