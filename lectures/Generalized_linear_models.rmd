...
- see also `DHARMa` package `simulateResiduals(model,plot=TRUE)`
...
- formula like `lm` (but specifies relationship on link scale)
...
```{r dharma}
library(DHARMa)
g1_nb <- MASS::glm.nb(cases~date,aids)
g0 <- update(g1, family=poisson)
plot(simulateResiduals(g1_nb))
```
