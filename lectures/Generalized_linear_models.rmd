...
<!-- stackoverflow.com/questions/37944197/ -->
<style>
.smallslide {
   font-size: 8px;
}
...
* for classic GLMs, this is all you need
...
> 	- = $\textrm{Poisson}(\lambda = \exp(0.5))=\textrm{Poisson}(\lambda=`r round(exp(0.5),3)`)$
...
curve(exp(x),from=-4,to=4,xlab="x (log-counts)",ylab="counts", main="inverse-link (exp)")
...
curve(plogis(x),from=-4,to=4,xlab="x (log-odds)",ylab="logistic(x)\n(probability)", main="inverse-link (plogis)")
curve(qlogis(x),from=plogis(-4),to=plogis(4),xlab="x (probability)",ylab="logit(x)", main="link (qlogis)")
par(op)
```
...
- Wald $Z$ tests (i.e., results of `summary()`), confidence intervals
	...
	- $Z$ rather than $t$ tests: "degrees of freedom" usually ingored
...
```{r perf1, fig.width = 8, fig.height = 12}
...
```{r inf2, class.output = "smallslide"}
