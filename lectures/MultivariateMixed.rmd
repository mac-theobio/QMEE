...
dll_data <- read.csv("../data/dll.csv") |>
  mutate(across(temp, factor),
         ## scale & center (and convert back from matrix to vector)
         across(any_of(morph_vars), ~drop(scale(.)), .names = "{.col}_s")
)
```
...
The way we're going to make this work is to expand ("melt", or `gather()` or `pivot_longer()` if we're using `tidyr`) the data set to be $mp$ observations long, then treat each individual (which was previously a single row of the data set but now comprises $p$ rows) as a group (we'll call this `units`):

## A trick to do multivariate mixed models using lme4

`lme4` does not (currently) have a natural syntax for multivariate responses, however, as I alluded to in class, there is an important relationship between multivariate response models and so called "repeated" measures (or longitudinal) models. As such we can use a few tricks to estimate the model in lme4. Below this, I will go through the same model using MCMCglmm, which is a library which has a more natural syntax for such multivariate responses, but is explicitly Bayesian, so you need to provide prior distributions. 
...
dll_melt <- (dll_data
    ...
    %>% pivot_longer(names_to = "trait", cols = paste0(morph_vars, "_s"))
    %>% drop_na()
    %>% arrange(units)
)
...
- the `system.time()` business is just to keep track of how long the command takes to run
(we could have kept the HTML output clean by hiding this in the `.Rmd` file, at the cost of making the `.Rmd` 
more complicated/confusing for those who were following along there)
...
(not so for GLMMs)
- `optimizer="bobyqa"` is the only choice of optimizers that does *not* give a convergence warning (ask 
about this if you feel like going down a rabbit hole)
...
- underlying var-cov parameters are labeled $\theta$: `getME(fit,"theta")` extracts them if you need them for something.
