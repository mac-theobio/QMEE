...
Because `light` has only two levels (sunny/shady), this gives the same $p$-value as shown in `summary()` for `lightsunny`
...
Set `options(contrasts = c("contr.sum", "contr.poly"))` if you want to use type 3 ANOVA!

```{r test3B, eval = FALSE}
car::Anova(lm_int, type = "II")
car::Anova(lm_int, type = "III")
lm_int_sum <- update(lm_int, contrasts = list(light = contr.sum, time = contr.sum))
