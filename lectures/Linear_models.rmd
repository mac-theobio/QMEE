...
- *Response variables* are linear functions of *input variables*, in turn based on *predictor variables*
     - One or more input variables per predictor variable
...
- Sensitive to some violations of assumptions - anomalous events have a larger effect than they should (**robust** models)
- Alternatives
...
in $a$ and $b$ (the unknowns)
        - Polynomial issues: use either *orthogonal polynomials* (`poly` in R, e.g. `y  ~ poly(x,2)`) or splines (`splines::ns()`, or `mgcv` package)
...
anova(lmint, lmlight, test="F")
## Test light
anova(lmint, lmtime, test="F")
## drop1 now tests the whole interaction, also very useful
drop1(lmint, test="F")
car::Anova(lmint)  ## 'type II'; drop interaction before testing
car::Anova(lmint, type='III')  ## 'type III'; test time at baseline light & v. versa
```
