...
R's behaviour is designed for testing *differences among groups*. To get the estimates per group, you could:
...
or
```{r dotwhisker,message=FALSE, warning = FALSE}
library("dotwhisker"); dwplot(lm0)
```
...
print(ggplot(mliz,aes(x=value,y=grahami))
      + geom_boxplot(,fill="lightgray")
      + facet_wrap(~variable,scale="free_x",nrow=1)
      + geom_hline(yintercept=mean(lizards$grahami),
                   colour="red",lwd=1,alpha=0.4))
```
...
xpos <- -0.1
print(ggplot(pp,aes(x=time,y=grahami,colour=light))
      + geom_point()
...
- [`gmodels::fit.contrast()`](https://search.r-project.org/CRAN/refmans/gmodels/html/fit.contrast.html) (show parameter estimates based on re-fitting models with new contrasts), [`rms::contrast.rms()`](https://search.r-project.org/CRAN/refmans/rms/html/contrast.html) (ditto, for `rms`-based fits)
