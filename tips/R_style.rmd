...
* See [“Code Smells and Feels.”](https://speakerdeck.com/jennybc/code-smells-and-feels), Jenny Bryan (2018), for some more advanced suggestions.

Additions from BMB and JD:

- **DRY** = "don't repeat yourself". Encapsulate common stuff in functions, or use tidyverse magic ([across](https://dplyr.tidyverse.org/reference/across.html))
- **never use absolute paths**
- don't use file names with spaces or parens
- don't mask built-in R objects (`t`, `c`, `data`, ...)
- don't depend on case sensitivity
- don't put `rm(list=ls())` at the top of your code (see [commentary by Jenny Bryan](https://tidyverse.org/blog/2017/12/workflow-vs-script/))
- use `TRUE` and `FALSE` rather than `T`/`F` (is this in the other style guides already???)
