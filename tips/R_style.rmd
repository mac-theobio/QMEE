...
- **don't** leave interactive commands like `View()`, `head()`, `str()` in your code [unless commented out]
- loading packages:
	...
	- don't load a huge list of unnecessary packages (e.g. all of the ones used in class examples); try to load just the ones you need
...
- **don't** use "extreme tidyverse"; e.g. `dd %>% pull(x) %>% mean()` instead of `mean(dd$x)`
