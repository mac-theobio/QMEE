...
we're interested in are `character`, `numeric` and `logical` (ie., `TRUE` or `FALSE`). `str()` is one way to try to figure out what your R object is.
...
-   Many people use `camelCase` or `underscor_spacing` ("snake case") or `dot.spacing` ("kebab case") to make readable variable names
    -   but it's also better not to use `variableNamesThatAreExcessivelyLong`
