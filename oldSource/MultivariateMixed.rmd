...
And we can take a look at how this has changed the structure of the data from a **wide** format:
```{r head_wide}
...
- the fit is a little slow (`r round(t1["elapsed"])` seconds the last time it was run (GLMMs would be even slower)
...
```{r tidy_ran_pars, warning=FALSE, message=FALSE, cache=TRUE}
...
```{r brms1,cache=TRUE, results="hide"}
library(brms)
t3 <- system.time(
    brmsfit <- brm(
...
This takes a long time! (`r timefun(t3)`), even computing the chains in parallel)
