...
for (i in 1:1000) {
    ...
    bdat <- transform(ants,
                      colonies=ants$colonies[sample(nrow(ants))])
...
Some alternative recipes for computing the difference in the means: (1) base R with `aggregate()` ...
...
For some reason they seem to use opposite signs
for the effect size:
```{r ant_t_test}
(tt <- t.test(colonies~place,data=ants,var.equal=TRUE))
```

We could substitute the test statistic here (diff/sd = `r round(tt$statistic,2)`) for the difference of means used above.
...
Load `gtools` package and generate the combinations, as in the original example:
```{r get_comb}
